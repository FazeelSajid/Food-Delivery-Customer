...
import {addShippingAddress} from '../../../utils/helpers/localStorage';
...
const ShippingAddress = ({navigation, route}) => {
  ...
  const addShippingAddressTODB = async () => {
    if (validate()) {
      setLoading(true);
      ...
      fetch(api.add_location, {
        ...
        .then(async response => {
          ...
          if (response?.status == true) {
            addShippingAddress(response?.result); // save in local storage
            navigation?.goBack();
          } else {
        ...
        .catch(err => {
          console.log('Error in addShippingAddressTODB :  ', err);
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  const handleAddress = () => {
    if (location_id) {
      ...
      console.log('addShippingAddressTODB');
      addShippingAddressTODB();
    }
  };
